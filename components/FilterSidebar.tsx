      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
        ...
        {activeCount > 0 && (
          <button
            ...
            className="text-[10px] font-bold uppercase tracking-wide text-amber-600 hover:text-amber-700 transition"
          >
            Clear all
          </button>
        )}
...
export default function FilterSidebar(props: FilterSidebarProps) {
  ...
  return (
    <>
      ...
      <button
        className="lg:hidden flex items-center gap-2 px-4 py-2.5 bg-white rounded-xl border border-slate-200 shadow-sm text-sm font-bold text-slate-800 hover:border-amber-400 transition"
        onClick={() => setDrawerOpen(true)}
      >
        <SlidersHorizontal size={15} className="text-amber-500" />
        Filters
        {activeCount > 0 && (
          <span className="ml-0.5 flex items-center justify-center w-5 h-5 rounded-full bg-amber-500 text-white text-[10px] font-black">
            {activeCount}
          </span>
        )}
      </button>
