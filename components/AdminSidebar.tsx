...
export default function AdminSidebar({ activeView, onViewChange, collapsed, onToggleCollapse }: AdminSidebarProps) {
  return (
    <aside
      className={`relative flex flex-col bg-[#0D0D0D] border-r border-[#C9A84C]/10 transition-all duration-300 min-h-screen ${collapsed ? "w-16" : "w-60"
        }`}
    >
      {/* Logo */}
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 text-sm font-sans tracking-wide transition-all duration-200 border-l-2 ${collapsed ? "justify-center" : ""
                } ${active
